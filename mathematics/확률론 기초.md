...
- **완전 독립성**
    ...
    **EX)** X1,X2~Bern(1/2)인 동전 던지기 게임이라고 하자. X1과 X2가 같은 면일 경우 이긴다고 하였을 때, 새로운 확률변수 X3는 아래 식과 같다.
    $$
    X3=\begin{cases}1,\;if\;X_1=X_2\\0,\;if\;X_1\neq X2\end{cases}
    $$
...
- **포아송분포 (Pois(lambda), Poisson distribution)**
  ...
  $$
  P(X=k)=e^{-\lambda}\frac {\lambda^k}{k!}, \space\space k=0,1,2,...
  $$
  ...
  - Poisson paradigm
    ...
    $$
    Events\space A_1, A_2,...,A_n,\space P(A_j)=p_j\space (n은\space 매우\space 크고,\space p_j는\space 작다.)\\
    만약\space events들이\space independent\space 또는\space weakly\space dependent라면\\ A_j의\space 발생\space 횟수는\space 포아송에\space 근사한다고\space 할\space 수\space 있다.  \\
    \lambda는\space A_j발생\space 횟수의\space 기대값이므로\space 아래\space 식으로\space 표현할\space 수\space 있다. \\
    Pois(\lambda), \lambda=\sum_{j=1}^{n}p_j
    $$

  - 이항분포와 포아송의 관계
    $$
    X\sim Bin(n,p),\space let\space n\rarr\infin, \space p\rarr0,\space \lambda=np\space is\space held \space constant\\
    \begin{aligned}
    Bin(n,p)&=\begin{pmatrix}n\\k\end{pmatrix}p^k(1-p)^{n-k}\\
    &=\frac{n(n-1)...(n-k+1)}{k!}(\frac{\lambda}{n})^k(1-\frac{\lambda}{n})^{n-k},\space (p=\frac{\lambda}{n})\\
    &=\frac{n(n-1)...(n-k+1)\lambda^k}{k!n^k}(1-\frac{\lambda}{n})^{n}(1-\frac{\lambda}{n})^{-k}\\
    &=\frac{\lambda^k}{k!}*e^{-\lambda}*1,\space ((1+\frac{x}{n})^n=e^x,\space if \space n\rarr\infin)\\
...
- **확률밀도함수 (PDF, Probability Density Function)**
  ...
  - PDF 조건 2가지
    $$
    1.\space f(X) \geq 0\\
    2.\space \int_{-\infin}^{\infin}f(x)dx=1
    $$

  - CDF from PDF
    $$
    F(x)=P(X\leq x)=\int_{-\infin}^xf(t)dt
    $$
  ...
  - PDF & CDF
    $$
    P(a\lt X\lt b)=\int_a^bf(x)=F(b)-F(a)
    $$
...
- **균등분포 (Unif(a,b), Uniform distribution)**
  ...
  - CDF
    $$
    \begin{aligned}F(x)=\int_{-\infin}^xf(t)dt&=\int_{a}^xf(t)dt\\
    &=\begin{cases}0,\;\;\;\;\;\;if\;x\lt a\\\frac{x-a}{b-a},\;if\;a\leq x\leq b\\1,\;\;\;\;\;\;if\;x\gt b
    \end{cases}
    \end{aligned}
    $$
  ...
  - 균등분포의 일반성 (Uniform is universal)
    ...
    $$
    Let\; U~Unif(0,1),\; F\; be\; a\; CDF\;(F는\; 연속함수이면서\; 강한\;증가함수라고\; 가정)\\
    Then \;Let\;X=F^{-1}(u), \;then\;X\sim F이다. (X의\; 누적분포함수는\; F가 된다.)\\
    증명) P(X\leq x)=P(F^{-1}(u)\leq x)=P(u\leq F(x))=F(x)\\
    따라서,\; X는\; F(x)의\; 분포를\; 따른다.
    $$

  - 균등분포의 대칭성
    $$
    u\sim Unif(0,1)=1-u\sim Unif(0,1)
    $$

  - 균등분포의 선형성
    $$
    a+bu\; 또한 \; 균등분포이다. (선형변환이기\; 때문에,\; 다만\;범위가\;달라진다.)
    $$
...
- **정규분포/가우시안 (Normal distribution)**
  ...
  - 표준정규분포 ( N(0,1) )
    ...
    $$
    \Phi(z)=\frac{1}{\sqrt{2\pi}}\int_{-\infin}^z e^{\frac{-t^2}{2}}dt
    $$
    ...
    $$
    f(z)=ce^{\frac{-z^2}{2}},\quad (c:정규화\;상수)
    $$
    ...
    - 정규화 상수 구하기
      ...
      $$
      \int_{-\infin}^\infin ce^{\frac{-z^2}{2}}dz=1
      $$
      ...
      $$
      \begin{aligned}
      \int_{-\infin}^\infin e^{\frac{-z^2}{2}}dz\int_{-\infin}^\infin e^{\frac{-z^2}{2}}dz\;& 두번을\; 곱해준다.\\
      \int_{-\infin}^\infin e^{\frac{-x^2}{2}}dx\int_{-\infin}^\infin e^{\frac{-y^2}{2}}dy&=\int_{-\infin}^\infin\int_{-\infin}^\infin e^{\frac{-(x^2+y^2)}{2}}dxdy\\
      &=\int_{0}^{2\pi}\int_{0}^\infin e^{\frac{-r^2}{2}}rdrd\theta\quad (Jacobian에\; 의해)\\
      &=\int_{0}^{2\pi}\int_{0}^\infin e^{-u}dud\theta\quad (u=\frac{r^2}{2})\\
      &=\int_{0}^{2\pi}1d\theta\\
      &=2\pi\\\\
      따라서, \int_{-\infin}^\infin &e^{\frac{-z^2}{2}}dz=\sqrt{2\pi}이\; 된다.\\
      c\sqrt{2\pi}=1&이므로,\; c=\frac{1}{\sqrt{2\pi}}가 된다.
      \end{aligned}
      $$
...
4. **포아송분포 기대값 증명**
   $$
   \begin{aligned}
   E(X)&=e^{-\lambda}\sum_{k=0}^\infin k\frac{\lambda^k}{k!}\\
   &=\lambda e^{-\lambda}\sum_{k=0}^\infin \frac{\lambda^{k-1}}{(k-1)!}\\
   &=\lambda e^{-\lambda} e^{\lambda}\quad (테일러\space 급수에\space 의해)\\
   &=\lambda
   \end{aligned}
   $$
...
1. **균등 분포 기대값, 분산 증명**
   $$
   \begin{aligned}E(X)&=\int_{-\infin}^{\infin}xf_X(x)dx\\
...
- **테일러 급수**
  ...
  <img src="../../../../AppData/Roaming/Typora/typora-user-images/image-20220106134507822.png" alt="image-20220106134507822" style="zoom:67%;" />

- **포아송 증명에 쓰이는 공식**
  $$
  (1+\frac {x}{n})^n=e^x,\space if\space n\rarr\infin
  $$
...
- **야코비안 (Jacobian)**
  ...
  ![image-20220106164140960](../../../../AppData/Roaming/Typora/typora-user-images/image-20220106164140960.png)

  아래 그림에서 보면 알 수 있듯이, 비선형 변환을 국소적으로 관찰하면 선형 변환인 것을 알 수 있다. 야코비안은 이러한 특징을 나타낸다. 

  <img src="../../../../AppData/Roaming/Typora/typora-user-images/image-20220106164207687.png" alt="image-20220106164207687" style="zoom:67%;" />

  하나의 예제를 들어보겠다. 
  $$
  dx,dy에서\;dr,d\theta로의 변환을\; 예로\; 들\; 수\; 있다.\\
  $$
  ![image-20220106164543954](../../../../AppData/Roaming/Typora/typora-user-images/image-20220106164543954.png)

  ![image-20220106164555460](../../../../AppData/Roaming/Typora/typora-user-images/image-20220106164555460.png)
  $$
  따라서,\;dxdy=|J|drd\theta=rdrd\theta라고\;할\;수\;있다.
  $$
