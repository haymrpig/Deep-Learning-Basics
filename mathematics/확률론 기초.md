...
- **Joint PDF**  
  ...
  - **독립**일 경우
    ...
    E(XY)=E(X)E(Y)의\; 증명\\
    \begin{aligned}
    E(XY)&=\int(\int xyf_X(x)f_Y(y)\;dx)dy\\
    (괄호를\; 먼저\; 계산한다고&\; 할\;때,\; y를\; 상수로\; 볼\; 수\; 있기\; 때문에\; 앞으로\; 뺄\; 수\; 있다. )\\
    &=\int yf_Y(y)\;(\int xf_X(x)dx)\;dy\\ ( 괄호\; 안은\; &E(X)와\; 같기 때문에 \; 앞으로 \; 뺄 \;수\; 있다.)\\
    &=E(X)E(Y)
  ...
  - **EX**) X,Y가 iid Unif(0,1)일 때, E(|X-Y|)를 구하라
    ...
    \begin{aligned}
    &\int\int |x-y|\;dxdy\\
    &={\int\int}_{x>y}(x-y) dxdy+{\int\int}_{x\leq y}(y-x) dxdy\quad (대칭이기에\; 아래\; 식으로\; 표현\; 가능)\\
    &=2\int_0^1\int_y^1(x-y)dxdy\quad (y는\; 0부터\; 1까지,\; x는\; y보다\; 커야함으로\; y부터\; 1까지)\\
    &=\frac {1}{3}
    \end{aligned}
    $$
  ...
  - **EX2)** N~Pois(lambda)의 달걀이 각각 p의 확률로 부화할 때, X를 부화하는 달걀의 개수, Y를 부화하지 않는 달걀의 개수라 할 때, X+Y=N을 만족하며, X|N~Bin(N,p)이다.
    $$
    \begin{aligned}
    P(X=i,Y=j)&=\sum_{n=0}^\infin P(X=i,Y=j|N=n)P(N=n)\\
    &=P(X=i,Y=j|N=i+j)P(N=i+j)\quad ( 1번\; 때문에)\\
    &=P(X=i|N=i+j)P(N=i+j)\quad (2번\; 때문에)\\
    &=\frac {(i+j)!}{i!j!}p^iq^j\frac {e^{-\lambda}\lambda^{i+j}}{(i+j)!}\\
    &=e^{-\lambda p}\frac {(\lambda p)^i}{i!}e^{-\lambda q}\frac {(\lambda q)^j}{j!}\quad (3번\; 때문에)\\\\
    1번)\;\;간단한\; 예로\; &\; X=1,\; Y=3,\; N=6이면\; 말이\; 안되는\; 상황이라는\; 것을\; 알\; 수\; 있다.\\
    모든\; 알의\; 개수가\; &6인데,\; 부화한\; 게\; 1,\; 부화 안한게\; 3이면\;나머지\; 2개는\; 설명이\; 안된다.\\\\
    2번)\;N=i+j이&기 때문에 Y=j항은 중복이다.\\\\
    3번)\;\; p+q=1이&기 때문에 e^{-\lambda}=e^{-\lambda (p+q)}
    \end{aligned}
    $$
