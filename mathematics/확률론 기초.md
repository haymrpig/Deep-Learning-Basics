...
- **목차**
  ...
  7. [**연속분포**](#7-연속분포)
     ...
     - 정규분포/가우시안 (Gaussian)
     - 지수분포 (Exponential)
  8. [**각 분포의 기댓값, 분산 증명**](#-각-분포의-기댓값,-분산-증명)
...
- **분산 (Variance)**
  ...
  - 분산의 성질
    ...
    1.\;Var(X+c)=Var(X)\\
    2.\;Var(cX)=c^2Var(X)\\
    3.\;Var(X+Y)\neq Var(X)+Var(Y),\quad if\;X\;and\;Y\; are\; not\; 
    independent\\
    (\;Var(X+Y)= Var(X)+Var(Y),\quad if\;X\;and\;Y\; are\; 
    independent)\\
    $$
...
- **정규분포/가우시안 (Normal distribution)**
  ...
  - **정규분포**의 **평균**과 **분산**
    $$
    Let\;X=\mu+\sigma Z,\;\mu\in R,\;\sigma \gt 0\\
    E(X)=\mu,\;\;Var(X)=Var(\mu+\sigma Z)=\sigma^2Var(Z)=\sigma^2
    $$
    ...
    - 표준화의 이용
      ...
      PDF는\; CDF의\; 미분이므로\; (기타\; 유용한\; 공식들 - 연쇄법칙을\; 이용한\; 미분)\\
      PDF = \frac{1}{\sigma\sqrt{2\pi}}e^{-\frac{(\frac{x-\mu}{\sigma})^2}{2}}이\;된다.
      $$
    ...
    - 정규분포의 합
      ...
      독립된 정규분포의 합은 정규분포이다. 
      $$
      X_j\sim N(\mu_j,\sigma_j^2)이고,\; independent할\; 때\\
      X_1+X_2\sim N(\mu_1+\mu_2,\sigma_1^2+\sigma_2^2),\\
      X_1+X_2\sim N(\mu_1-\mu_2,\sigma_1^2+\sigma_2^2)이다.
      $$
    ...
    - 68-95-99.7%의 법칙
      ...
      P(|X-\mu|\leq 2\sigma)&\approx0.95\\
      P(|X-\mu|\leq 3\sigma)&\approx0.997
      \end{aligned}
      $$
...
- **지수분포 (Expo(lambda), Exponential distribution)**
  ...
  - 지수분포의 응용
    $$
    Let\; Y=\lambda X,\; then \;Y\sim Expo(1)이다. \\
    위\;식이\;성립할\;경우\;매우\;쉽게\;지수분포를\;사용할\;수\;있다.\\
    \begin{aligned}\\
    증명)\\
    &P(Y\leq y)=P(X\leq \frac{y}{\lambda})=1-e^{-\lambda\frac{y}{\lambda}}=1-e^{-y}\\
    &즉,\; Expo(1)=1-e^{-y}이므로 성립한다.
    \end{aligned}
    $$
  ...
  - 지수분포의 무기억성
    ...
    $$
    즉,P(X\geq s+t|X\geq s )=P(X\geq t)이다.
    $$
    ...
    $$
    \begin{aligned}
    &P(X\geq s)(X\leq s)=e^{-\lambda s}\\\\
    P(X\geq s+t|X\geq s )&=\frac{P(X\geq s+t,X\geq s)}{P(X\geq s)}\\
    &=\frac{P(X\geq s+t)}{P(X\geq s)}\quad (X\geq s와\; X\geq s+t는\; 겹치기\;때문에)\\
    &=\frac {e^{-\lambda (s+t)}}{e^{-\lambda s}}\\
    &=e^{-\lambda t}\\
    &=P(X\geq t)\\\\
    따라서, P(X\geq &s+t|X\geq s )=P(X\geq t)이다. 
    \end{aligned}
    $$
    **EX)**
    $$
...
$$
이항분포의\;분산은\; 정의에\; 의해서\; 구할\;수\;있지만,\; 복잡하다.\\
대신,\; 이항분포가\; 독립적인\; 베르누이\; 분포의\; 합인것을\; 이용하여\; 구한다.\\
분산의\; 성질에\; 의하여\; 독립적인\; 분포의\; 경우\; Var(X+Y)=Var(X)+Var(Y)가\; 성립하므로,\\
X=I_1+...+I_n,\;I_j\sim iid\;Bern(p)\\
X^2=I_1^2+...I_n^2+2I_1I_2+2I_1I_3+...+2I_{n-1}I_n\\
\begin{aligned}E(X^2)&=nE(I_1^2)+2\begin{pmatrix}n\\2\end{pmatrix}E(I_1I_2)\\
&=np+n(n-1)p^2\\
\\\quad (I_1=1또는\;0이므로\;\; &I_1^2=1또는\;0이다, 즉, E(I_1^2)=E(I_1)=p이다.\\
2\begin{pmatrix}n\\2\end{pmatrix}=n(n-1)&이고 \;I_1I_2\;둘\;중\;하나라도\;0이면\;I_1I_2=0이다.\\ 따라서,\; I_1I_2=1인\; &경우는\; 둘\; 다\;\; 1인\; p*p인\; 확률을\; 가진다.)\\\\
Var(X)&=E(X^2)-E(X)^2\\
&=n^2p^2-np^2+np-(np)^2\\
&=-np^2+np\\
&=np(1-p)\\
...
4. **포아송분포 기대값, 분산 증명**  
   ...
   $$
   \begin{aligned}
   E(X^2)&=\sum_{k=0}^\infin k^2\frac{e^{-\lambda}\lambda^k}{k!}\\
   &=\lambda e^{-\lambda}\sum_{k=0}^\infin \frac{k^2\lambda^{k-1}}{k!}\\
   &=\lambda e^{-\lambda}(\lambda+1)e^{\lambda}\\
   &=\lambda(\lambda+1)\\\\\\\\\\
   Var(X)&=E(X^2)-E(X)^2\\
   ...
   \end{aligned}
   \begin{aligned}
   \;\;\;\;\ \Leftarrow(\quad&\sum_{k=0}^\infin \frac{\lambda^k}{k!}=e^\lambda,\;테일러급수에\; 의해\\
   &\sum_{k=0}^\infin \frac{k\lambda^{k-1}}{k!}=e^\lambda,\;양변\;미분\\
   &\sum_{k=0}^\infin \frac{k\lambda^{k}}{k!}=\lambda e^\lambda,\;양변 \lambda 곱하기\\
   &\sum_{k=0}^\infin \frac{k^2\lambda^{k-1}}{k!}=(\lambda+1) e^\lambda,\;양변\;미분\quad)
   \end{aligned}
   $$
...
2. **지수분포의 기댓값, 분산 증명**
   ...
   \begin{aligned}
   E(Y)=\int_0^{\infin}ye^{-y}dy&=(-ye^{-y})|_0^{\infin}+\int_0^\infin e^{-y}dy\\
   &=1\\
   Var(Y)=E(Y^2)-E(Y)^2&=\int_0^{\infin}y^2e^{-y}dy-1\\
   &=1\\\\
   따라서, \; X&=\frac{Y}{\lambda}이므로\\ E(X)=\frac{1}{\lambda},& Var(X)=\frac{1}{\lambda^2}
   \end{aligned}
   $$
...
- **이산 데이터에서의 LOTUS (무의식적인 통계학자의 법칙) 증명하기**
  $$
  \begin{aligned}
  Show\; E(g(X))&=\sum_xg(x)P(X=x)\quad (grouped된\; 걸로\; 볼\;수\;있다.)\\
  &=\sum_{s\in S}g(X(s))P(\{s\})\quad (ungrouped된\; 것으로\; 볼\; 수\; 있다.)\\
  &=\sum_x\sum_{S:X(s)=x}g(x)P(\{s\})\\
  &=\sum_xg(x)\sum_{S:X(s)=x}P(\{s\})\quad (g(x)는\; S와\; 연관되어\; 있지\; 않으므로)\\
  &=\sum_xg(x)P(X=x)
  \end{aligned}
  $$
  ...
  - 예제
    ...
    E(Y)=\int_0^1e^xf(x)dx로\; 식을\; 세우고\\ X의\; CDF로부터\;PDF를\; 찾아서\; 계산할\; 수도\; 있지만,\;이는\; 복잡하다. \\
    P(u^2\leq x)=P(u\leq \sqrt x)=\sqrt x,\; if\; 0\lt x\lt 1\\
    f(x)=\frac{1}{2}x^{-\frac{1}{2}}가\; 된다. \\\\
    \\더 \;좋은 \;방법은 :\\
    E(Y)=\int_0^1e^{u^2}du로\; 적는\; 것이다. \\
    (0\sim1범위에서\; 균등분포는\; 1인\;것을\; 알기\; 때문에\; 이렇게\; 적을\; 수\; 있는\; 것이다. )
    $$
...
- **대칭성과 선형성 예제**
  $$
  X,Y,Z\;be\;iid,\; positive\; random\; variables,\\
  Find\;E(\frac{X}{X+Y+Z})=E(\frac{Y}{X+Y+Z})=E(\frac{Z}{X+Y+Z}),\;\;(대칭성)\\
  \begin{aligned}
  E(\frac{X}{X+Y+Z})+E(\frac{Y}{X+Y+Z})+E(\frac{Z}{X+Y+Z})&=E(\frac{X+Y+Z}{X+Y+Z})\;\; (선형성)\\
  &=E(1)\\
  &=1\\
  따라서, \frac {E(\frac{X}{X+Y+Z})}{3}=\frac{1}{3}
  \end{aligned}
  $$
